...
const handleAddFactorClick = (index) => {
  ...
  newRows[index].factors.push({ 
    name: (
      <textarea
        id={`factor${newRows[index].factors.length + 1}`}
        name={`factor${newRows[index].factors.length + 1}`}
        className="custominput"
        rows="6"
        cols="20"
        placeholder={`Factor ${newRows[index].factors.length + 1}`}
      ></textarea>
    ),
    statuses: [{ name: <input type="text" placeholder={`Status 1`} />, id: 1 }],
  });
  ...
  const newRow = {
    result: (
      <textarea
        ...
        name={`result${rows.length + 1}`}
        className="custominput"
        rows="6" // Atur jumlah baris sesuai kebutuhan
        cols="20" // Atur lebar kolom sesuai kebutuhan
        placeholder={`Result ${rows.length + 1}`}
      ></textarea>
    ),
    factors: [
      {
  ...
  return (
    <div className="table-container">
      <table className="safety-table">
        <caption>Edit & Upload Table</caption> {/* Judul tabel */}
        <thead>
          <tr>
            <th rowSpan="2">
              KPI Result
...
export default EditUploadTable;
